	...
	if(i==20)
        ...
        while (1) {
            ...
            if (activity > 0 && FD_ISSET(client_fd, &readfds)) {
                ...
                sscanf(buffer, "CPU: %f | MemTotal: %f | MemFree: %f", &cpu_usage, &mem_total, &mem_free);
                printf("CPU USAGE LVGL: %.2f\n", cpu_usage);
                float ddr_usage_f = ((mem_total - mem_free) / mem_total);
//                int ddr_usage = ((mem_total - mem_free) / 10240);
                int ddr_usage = ddr_usage_f * 100;
...
static void charging_time_until_full_label_observer(lv_observer_t * observer, lv_subject_t * subject)
{
    ...
    lv_label_set_text_fmt(label, "%"LV_PRId32"%%", v_range_time_to_full);
}
...
static void create_widget_charging(lv_demo_high_res_ctx_t * c, lv_obj_t * widgets)
{
    ...
    lv_subject_add_observer_obj(&global_api->subjects.ddr_usage_sub, charging_percent_label_observer, percent_label, NULL);
    ...
    lv_obj_add_style(time_to_full_num_label, &c->fonts[FONT_LABEL_XL], 0);
    lv_obj_set_style_text_color(time_to_full_num_label, lv_color_white(), 0);
    lv_subject_add_observer_obj(&global_api->subjects.ddr_usage_sub, charging_time_until_full_label_observer, time_to_full_num_label,
                                NULL);
}
...
static lv_obj_t * create_widget3_info(lv_demo_high_res_ctx_t * c, lv_obj_t * parent, const lv_image_dsc_t * img_dsc,
    const char * text, const char * unit, const char *value, const int flag)
{
...
if(setting == setting1) {
    ...
    } else if (code == LV_EVENT_CLICKED && !button_pressed) {
        printf("CALLED JAILHOUSE START\n");
        jailhouse_started = 1;
        lv_subject_set_int(&global_api->subjects.is_running_sub, 1);
        pthread_create(&hypervisor_thread, NULL, hypervisor_init, global_api);
...
//    lv_label_set_text_static(title_label, "Inmate");
    ...
    lv_obj_t * setting1 = create_widget3_setting(c, setting_box, c->imgs[IMG_COLD_ICON], "Start", true);
    lv_obj_t * setting2 = create_widget3_setting(c, setting_box, c->imgs[IMG_DRY_ICON], "Crash", false);
...
//    lv_obj_align(start_btn, LV_ALIGN_CENTER, 0, 0);
    ...
    */
    ...
    create_widget3_info(c, info_box, c->imgs[IMG_TIME_ICON], "CPU cores", "", "0", 1);
    create_widget3_info(c, info_box, c->imgs[IMG_ENERGY_ICON], "DDR memory", "MB", "0", 0);
